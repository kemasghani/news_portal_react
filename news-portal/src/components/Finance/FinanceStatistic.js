 ...
 */
class FinanceStatistic extends PureComponent {
    constructor(props) {
        super(props);
        this.label = ""; // Symbol
    ...
    render() {
        //console.log("[FinanceStatistic.js] render");  // for debug

        if (this.props.response) {
            this.label = this.props.response["Meta Data"]["2. Symbol"];
            this.lastUpdatedDateTime = this.props.response["Meta Data"][
                "3. Last Refreshed"
            ];
            let timeSeries = this.props.response["Time Series (5min)"];
            // filter labels by same date
            this.labels = Object.keys(timeSeries).filter(
                (datetime) =>
                    datetime.split(" ")[0] === this.lastUpdatedDateTime.split(" ")[0]
            );
            // take the close values sliced by same date
            this.data = Object.values(timeSeries)
                        ...
                        <div>
                            ...
                            <Card.Text>
                                <small className="text-muted">
                                    Updated {this.lastUpdatedDateTime}
                                </small>
                            </Card.Text>
                        </div>
                    )}
